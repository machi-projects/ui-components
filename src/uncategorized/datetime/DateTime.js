...
class DateTime extends React.Component {
	...
	render(){
		...
		return <div className={style.dateContainer} onClick={this.closePopup}>
					<i className={position?style[`${position}ArowPos`]:style.topArow}></i>
					<div  className={style.datemain}>

						<div className={style.optionsTop}>
							<span data-testId="prevYear" className={style.sideArows} onClick={this.modifyCalendar.bind(this,"ll")}>&lt;&lt;</span>
							<span data-testId="prevMonth" className={style.sideArows} onClick={this.modifyCalendar.bind(this,"l")}>&lt;</span>
							<span data-testId="resultString" className={style.monthStr}>{showmonthtxt}</span>
							<span data-testId="nextMonth" className={style.sideArows} onClick={this.modifyCalendar.bind(this,'g')}>&gt;</span>
							<span data-testId="nextyear" className={style.sideArows} onClick={this.modifyCalendar.bind(this,'gg')}>&gt;&gt;</span>
						</div>
						...
						<CalendarComponent date={date} year={year} month={month}  onSelect={this.dateSelect}/>
						{
							isDateTimeField && <div>
									<span className={style.timeStr}>{ i18NProviderUtils.getI18NValue('crm.events.time')}</span>
									<span className={style.dropDown}>
										<DropdownComponent id="hours" suggestions={this.hourSuggestions} value={ hours } valueField="id" textField="name" onSelect={ this.hoursSelect}/>
									</span>
									<span className={style.dropDown}>
										<DropdownComponent id="minutes" suggestions={this.minSuggestions} value={ mins } valueField="id" textField="name" onSelect={ this.minutesSelect }/>
									</span>
									<span className={style.dropDown}>
										<DropdownComponent id="amPm" suggestions={this.ampmSuggestions} value={ amPm } valueField="id" textField="name" onSelect={ this.amPmSelect }/>
									</span>
								</div>
						}

						<div className={style.marTop} >
								<button data-testId="submit" className={style.blueBut} onClick={this.handleSelect}>{ i18NProviderUtils.getI18NValue("support.request.history.value.update.set")}</button>
								<button data-testId="clear" className={style.canButton} onClick={this.handleClear}>{ i18NProviderUtils.getI18NValue("support.request.clear")}</button>
						</div>
					</div>
				</div>
	}
	...
	handleSelect(e){
		e.preventDefault();
		let { isDateTimeField,timeZone,min,max,onError,onSelect,minErrorText,maxErrorText }= this.props;
		...
		let dateString= isDateTimeField? `${year} ${month+1} ${date} ${hours} ${mins} ${amPm}`: `${year} ${month+1} ${date}`;
		let seletedDateObj= isDateTimeField? moment.tz(dateString,"YYYY MM DD hh mm A", timeZone):  moment.tz(dateString,"YYYY MM DD", timeZone);
...
function mapStateToProps(state,props){
	let timeZone= selectn("userDetails.timeZone",state);
	return { timeZone}
}
